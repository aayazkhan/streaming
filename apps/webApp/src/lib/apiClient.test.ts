...
describe("apiClient", () => {
  ...
  it("throws ApiError with the server's error fields on failure", async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue(
        jsonResponse(404, { code: "NOT_FOUND", message: "missing", correlationId: "abc" }),
      ),
    );
    await expect(apiClient.get("/missing", { auth: false })).rejects.toMatchObject({
      code: "NOT_FOUND",
      status: 404,
    });
  });
  ...
  it("refreshes the access token once and retries after a 401, then succeeds", async () => {
    ...
    const fetchMock = vi
      .fn()
      // first call: protected request fails with 401
      ...
      // third call: retried protected request succeeds
